...
class CounterReplacementBittrexXChangeTest extends FlatSpec with Matchers with TestLoggingSupport with BeforeAndAfter {
  ...
  private def createExchange(): Unit = {
    ...
    exchange = new CounterReplacingXChangeWrapper(underlying, List(CurrencyPair(Currency.USDollar, Currency.Bitcoin), CurrencyPair(Currency.Euro, Currency.Bitcoin)))
  }
  ...
  it should "get balance in other than replace currency" in {
